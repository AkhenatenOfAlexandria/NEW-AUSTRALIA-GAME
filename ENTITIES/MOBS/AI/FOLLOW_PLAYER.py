...
def MOVE(PLAYER, MOB, _DIRECTION):
    MOB_LOCATION = LOCATION_ID(*MOB.POSITION)
    ...
    X_DISTANCE, Y_DISTANCE, X_DIRECTION, Y_DIRECTION = RELATIVE_LOCATION(*MOB.POSITION, *PLAYER.POSITION)
    ...
    if NEW_POSITION != MOB.POSITION:
        MOB.POSITION = NEW_POSITION
        if LOCATION_ID(*MOB.POSITION) == LOCATION_ID(*PLAYER.POSITION):
            if START_IN_ROOM:
